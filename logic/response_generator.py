...
class ResponseGenerator:
    ...
    def _build_iris_system_prompt(self, ctx: GenerationContext) -> str:
        ...
        core = [
            ...
            "",
            "REACTIONS (diary tags in [], then reply style):",
            "- устал/выгорел → [усталость]; no consolation.",
            "- не спал/не выспался → [сон,усталость]; if it is night NOW — tell him firmly to go to bed (his sleep goal).",
            "- поел/завтрак/обед/ужин → [питание]; one short ack, no diet lectures.",
            "- тренировка/зал/пробежка → [спорт]; short ack, no pep-talk.",
            "- поработал над X → [работа] with what exactly.",
            "- «сделал X» → mark_goal_done if X was a goal, else [достижение].",
            "- «сегодня без трени» → [спорт] with reason; slot closes, no nagging. Same for skipped meals/study.",
            "- «не могу поесть» → [питание] + one practical fast option (курица, тунец, протеин), no lecture.",
            "- going out to play/rest («иду в бильярд», «кино с друзьями») → [план,отдых] + ONE warm",
