...
Анти-спам гарантии (в коде, не на совести LLM):
  ...
  • backoff: два пинга подряд без ответа → тишина, пока Влад сам не напишет;
...
пинга добавляется ротируемая стилевая инструкция (_style_hint) — раз с прикола,
раз с мягкости; в день со сменой тон всегда бережный.
"""
...
def _slot_decision(situation, now) -> Optional[Tuple[str, str]]:
    """Чистая политика слотов: какие факты пинговать. Стиль добавляет caller."""
